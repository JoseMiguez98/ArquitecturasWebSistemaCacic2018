...
//	}
	...
	protected void setUp() throws Exception{
		...
		UserDAO.getInstance().persist(walter, em);
		UserDAO.getInstance().persist(jesse, em);
		UserDAO.getInstance().persist(gus, em);
		UserDAO.getInstance().persist(saul, em);
		UserDAO.getInstance().persist(mike, em);
		UserDAO.getInstance().persist(hank, em);
		UserDAO.getInstance().persist(tuco, em);
		UserDAO.getInstance().persist(hector, em);
		UserDAO.getInstance().persist(skinny, em);
		UserDAO.getInstance().persist(brandon, em);
		
		ProjectDAO.getInstance().persist(cloud, em);
		ProjectDAO.getInstance().persist(dataMining, em);
		ProjectDAO.getInstance().persist(soundProcessing, em);
		ProjectDAO.getInstance().persist(imageProcessing, em);
		ProjectDAO.getInstance().persist(arduino, em);
		ProjectDAO.getInstance().persist(ia, em);
		ProjectDAO.getInstance().persist(ux, em);
		ProjectDAO.getInstance().persist(methCook, em);
		ProjectDAO.getInstance().persist(deepLearning, em);
		ProjectDAO.getInstance().persist(security, em);
	}
	...
	public void testCreateTenUsers() {
		List<User>users = UserDAO.getInstance().findAll(em);
	...
	public void testCreateTenProjects() {
		List<Project>projects = ProjectDAO.getInstance().findAll(em);
	...
	public void testConsultarDatosDeAutorRevisor() {
		User user = UserDAO.getInstance().findById(1, em);
		String user_data = user.toString();
		String user_data_from_database = UserDAO.getInstance().getData(1, em);
	...
	public void testGetTodosLosTrabajosAsignadosARevisor() {
		User user = UserDAO.getInstance().findById(25, em);
		assertNotNull(user);
		...
		List<Project>user_projects_from_database = UserDAO.getInstance().getRevisions(25, em);
	...
	public void testGetRevisionesRangoFechas() throws ParseException {
		...
		List<Project>user_projects = UserDAO.getInstance().getRevisionsBetween(1, "02/10/2016", "05/11/2017", em);
	...
	public void testGetTrabajosEnviados() {
		List<Project>projects=UserDAO.getInstance().getAllResearchWorks(10, em);
	...
	public void testGetInformacionTodosLosTrabajos() {
		...
		List<String>projects_info_from_database = ProjectDAO.getInstance().getAllResearchWorksInfo(em);
	...
	public void testGetTrabajosEnAreaEspecifica() {
		...
		List<Project>projects_from_database = UserDAO.getInstance().getAllResearchWorksOnArea(25, temas_generales[1], em);
